...
def div_train_and_test_file(object_instances_list):
    ...
    cord_instance_test, cord_instance_learn = div_and_shuffle_by_category(cord_instances)
    formation_instance_test, formation_instance_learn = div_and_shuffle_by_category(formation_instances)
    phone_instance_test, phone_instance_learn = div_and_shuffle_by_category(phone_instances)
    division_instance_test, division_instance_learn = div_and_shuffle_by_category(division_instances)
    product_instance_test, product_instance_learn = div_and_shuffle_by_category(product_instances)
    ...
    all_instance_learn = [cord_instance_learn, formation_instance_learn, phone_instance_learn,
                          division_instance_learn, product_instance_learn]

    export_to_xml_file(all_instance_test, "test.xml")
    export_to_xml_file(all_instance_learn, "train.xml")
...
def div_and_shuffle_by_category(object_instances_by_category_list):
    instance_train = {}
    while len(instance_train) < 50:
        index = randrange(0, len(object_instances_by_category_list))
        instance_train[index] = object_instances_by_category_list[index]

    for value in instance_train.values():
        object_instances_by_category_list.remove(value)

    print(len(object_instances_by_category_list), "----", len(instance_train))
    return object_instances_by_category_list, list(instance_train.values())


def main():
    start = time.clock()
    instances_list = instance_parsing("line.S2.data.clean.xml")
    div_train_and_test_file(instances_list)
